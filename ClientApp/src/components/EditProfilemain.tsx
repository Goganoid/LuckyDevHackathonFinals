...
import { SpecialTag } from './CreateProjectPopup';
...
export default function EditProfilemain() {
    ...
    const [skillTags, setSkillTags] = useState<SpecialTag[]>([]);
    ...
    useEffect(() => {
        Promise.all([UserApi.GetUserInfo(`${getUserId()}`)]).then(responses => {
            ...
            console.log(UserInfoResponse.data.skillTags.map(t => { return { label: t.label } as SpecialTag }));
            setSkillTags(UserInfoResponse.data.skillTags.map(t => { return { label: t.label } as SpecialTag }))
        });
            ...
            <MDBCard className='MDBCard p-4 m-3'>
                <MDBCardBody>
                    ...
                    <MDBCard>
                        <MDBCardBody>
                            <Select
                                value={skillTags.length !== 0 ? skillTags.map(t => {
                                ...
                                onChange={(newValue, { action }) => {
                                    ...
                                    if (action === 'select-option' || action === 'remove-value') {
                                        let newTags = newValue.map(nv => nv.value);
